...
def initialize_nltk():
    ...
    try:
        ...
        st.error(f"NLTKの初期化に失敗しました: {e}")

def calculate_metrics(answer, correct_answer):
    """評価指標を計算"""
    if not answer or not correct_answer:
        return 0.0, 0.0, len(answer.split()) if answer else 0, 0.0
    try:
        # BLEUスコア
        reference = [nltk.word_tokenize(correct_answer.lower())]
    ...
    try:
        # コサイン類似度
        vectorizer = TfidfVectorizer()
        tfidf_matrix = vectorizer.fit_transform([answer, correct_answer])
        similarity_score = cosine_similarity(tfidf_matrix[0:1], tfidf_matrix[1:2])[0][0]
    except Exception:
        similarity_score = 0.0
    # 単語数
    word_count = len(answer.split())
    # 関連性スコア（ここでは類似度スコアの簡易版）
...
def get_metrics_descriptions():
    """評価指標の説明を取得"""
    return {
        "BLEUスコア": "生成された回答と参照回答の文法的類似性を測定（0-1）。",
        "類似度スコア": "TF-IDFに基づくコサイン類似度で、意味的類似性を測定（0-1）。",
        "単語数": "回答中の単語数。",
        "関連性スコア": "質問または参照回答との関連性を測定（ここでは類似度スコアを使用）。",
        "効率性スコア": "正確性を応答時間（0.1を加算してゼロ除算を回避）で割った値。単位時間あたりのパフォーマンスを示す。"
    }
